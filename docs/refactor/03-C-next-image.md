...
## 6-B. 확장 — 캐롤셀 잔여 jpeg의 실체와 완결 (2026-07-29 추가)

**정정**: §5에서 "4번째 `<img>`"로 기록한 잔여 jpeg는 `<img>`가 아니라 **`next/image`에 `unoptimized`가 붙어 옵티마이저를 우회**한 것이었다. 캐롤셀 슬라이스 안에 2곳:
`LocationImageGallery.tsx`(갤러리)와 `LocationImageThumbnails.tsx`(썸네일).

측정으로 드러난 함정 — **갤러리만 고친 중간 상태에서 bytes가 오히려 증가했다**:

| `/content/1` 이미지                   | bytes                                         | 구성                                                                                                 |
| ------------------------------------- | --------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| after (3곳 전환, `ca61b05`)           | 632,341                                       | avif 1 + **원본 jpeg 579,920**                                                                       |
| 갤러리만 unoptimized 제거 (`81b1096`) | **712,697 (+13%)**                            | avif 2 + **원본 jpeg 579,920** — 갤러리는 avif가 됐지만 썸네일이 같은 원본을 계속 직로드 → 이중 로드 |
| 썸네일까지 제거 (`a82548d`)           | **163,074 (−74% vs after, −88.7% vs before)** | **avif 3** (썸네일은 w=384로 협상돼 30KB)                                                            |

원본 기록: `measurements/81b1096/C-after2-carousel/`(중간), `measurements/a82548d/C-after2-carousel-full/`(최종). `/location/1`은 캐롤셀이 없어 불변(231,775B, avif 4) — 교차 검증.

교훈: 같은 원본을 쓰는 컴포넌트 중 **하나만** 최적화하면 옵티마이저 사본과 원본이 둘 다 로드된다. `unoptimized`는 전환 대상 탐색(`<img>` grep)에 잡히지 않는 사각지대다.

## 7. 결론과 다음 행동

- **채택: B안 (next/image 전환 유지) + 캐롤셀 unoptimized 2곳 제거** — 근거: §5 판정 기준 충족, §6-B로 완결.
- 후속:
  1. 잔여 `unoptimized` 3곳 — `PosterCard`(홈), `PlaceThumbnail`(사이드바), `Modal/ContentCard`. 측정 라우트 밖이라 이번 범위에서 제외, §6-B와 같은 방식으로 전환 가능
  2. ~~라이브 백엔드 복구 시 외부 호스트 이미지로 1회 재검증~~ → **[종결 2026-07-29]** 남의 인프라 의존이라 무기한 미결이 될 수 있어 결정으로 닫는다: **백엔드가 복구되지 않으면 검증 생략을 확정**하고, MSW 고정 픽스처 측정을 이 실험의 유효 결과로 유지한다. 복구가 실제로 일어나면 그때 1회 재검증 (조건부, 대기 항목 아님)
  3. `/location/1` LCP 이봉 분포(5.5s/9.7s) — **미해결로 유지** (조사하지 않기로 결정, 2026-07-29)

## 8. 이력서 문장 (goals.md 이력서 지표)

- MSW 고정 픽스처로 측정 환경을 통제하고 next/image 전환 효과를 실측 — 이미지 전송량 최대 89% 절감(avif 협상), 포스터 LCP 9.6s→4.4s(−54%) 개선
- 이미지 88% 경량화에도 LCP가 불변인 화면을 함께 기록 — 전송량과 LCP의 비선형 관계를 실측으로 규명
- 부분 최적화가 이중 로드로 bytes를 +13% 늘리는 함정을 측정으로 검출·완결 — "고쳤다"가 아니라 재측정으로 증명하는 프로세스의 가치 입증
